...
import Model from '#models/shopify.credential.model'

import stringHelpers from '@adonisjs/core/helpers/string'
import database from '@adonisjs/lucid/services/db'

export default class ShopifyCredentialRepository {
  constructor() {}
  ...
  async findBy({ clause, ...payload }: Find<typeof Model>) {
    return await database.transaction(async function (client) {
      ...
      const raw = keys
        .flatMap((key) => ` "shopify_credential"."${stringHelpers.snakeCase(key)}" = ? `)
        .join(` ${clause} `)
