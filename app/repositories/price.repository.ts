...
import Model from '#models/price.model'
...
export default class PriceRepository {
  ...
  async createMany(payload: Create<typeof Model>[]) {
    return await database.transaction(async function (client) {
      return await Model.createMany(payload, { client })
    })
  }
  ...
  async updateMany(payload: Update<typeof Model>[]) {
    return await database.transaction(async function (client) {
      return await Model.updateOrCreateMany('id', payload, { client })
    })
  }
  ...
  async findBy({ clause, ...payload }: Find<typeof Model>) {
    ...
    const raw = keys
      .flatMap((key) => ` "prices"."${stringHelpers.snakeCase(key)}" = ? `)
      .join(` ${clause} `)
